...
export default class Search extends Component {
 ...
 onSearchText(){
    this.setState({isLoading: true}, async () => {
      ...
      let searchResultByBookName = await DbQueries.querySearchBookWithName("IRV", "Hin",this.state.text);
      if(searchResultByBookName && searchResultByBookName.length >0 ){
       ...
       let searchResultByVerseText = await DbQueries.querySearchVerse("IRV","Hin",this.state.text)
